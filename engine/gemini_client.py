"""
engine/gemini_client.py - Unified Gemini 3.8 Flash Client
Text & Multimodal Vision client with thinkingConfig, header-based auth, and graceful fallback.
"""
...
def generate_content(
    ...
    - thinking_level: 'low' or 'medium' (never 'minimal').
    ...
    safe_thinking = thinking_level if thinking_level in ("low", "medium") else None
        ...
        try:
            ...
            if e.code == 400 and safe_thinking:
                ...
                try:
                    ...
                    print(f"[Gemini Client] Retry without thinking failed on {model_name}: {retry_err}")
            elif e.code in (429, 500, 502, 503, 504):
                print(f"[Gemini Client] HTTP {e.code} on {model_name}, trying next fallback model...")
                time.sleep(1.0)
            else:
                print(f"[Gemini Client] HTTP {e.code} on {model_name}: {e}")
        except Exception as e:
            print(f"[Gemini Client] Error on {model_name}: {e}")
            time.sleep(0.5)
...
def _extract_text_from_candidate(resp_data: Dict[str, Any]) -> Optional[str]:
    """Extracts final text from Gemini response structure."""
    try:
