...
# Universal Semantic Visual Ontology hoisted to module scope
SEMANTIC_VISUAL_MAP = {
...
def semantic_fallback_plan(
    ...
    """
    if beats is None:
        beats = create_story_beats(script_text, total_duration, min_dur=2.0, max_dur=4.5)
    if not beats:
    ...
    for idx, (scene_text, dur) in enumerate(beats):
        ...
        # Check semantic ontology match using module-level SEMANTIC_VISUAL_MAP
        ...
        if matched_concept:
            ...
            must_show = [w for w in sq.split() if len(w) > 2]
            must_not_show = ["map", "route", "chart", "diagram", "technical drawing", "blurry", "watermark", "scanned document", "text overlay"]
            subject_anchor = subject_desc
        else:
            # When no ontology keyword matches, use the narration sentence itself as the subject
            subject_anchor = scene_text.strip().rstrip(".,!?;:")
            ...
            elif any(w in s_lower for w in ["small", "silver", "gold", "hand", "finger", "face", "eye", "close", "key", "coin", "screen", "bolt", "bolts", "spring"]):
            ...
            must_show = [w for w in filtered_kws[:3]] if filtered_kws else ["subject"]
            must_not_show = ["map", "route", "chart", "diagram", "technical drawing", "blurry", "watermark", "scanned document", "text overlay"]
        ...
        vp = f"Cinematic {motion} camera motion as {subject_anchor} moves naturally within the scene."
...
def plan_visual_storyboard(
    ...
    if beats is None:
        beats = create_story_beats(script_clean, total_duration, min_dur=2.5, max_dur=5.5)
    if not beats:
    ...
    if raw_text:
        ...
        if parsed and "scenes" in parsed and isinstance(parsed["scenes"], list):
            ...
            for idx, (b_text, b_dur) in enumerate(beats):
                ...
                vp = s_data.get("video_prompt") or f"Cinematic {motion} camera motion as {b_text[:40]} unfolds in the scene."
